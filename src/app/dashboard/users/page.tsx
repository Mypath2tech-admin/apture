...
import { AlertCircle, UserPlus, Search, RefreshCw } from "lucide-react"
...
export default function OrganizationUsers() {
    ...
    const isAuthorized = user?.role === "ADMIN" || user?.role === "ORGANIZATION_ADMIN" || user?.role === "ORGANIZATION_MEMBER"
    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            toast.error(err instanceof Error ? err.message : "Failed to load organization users")
        } finally {
    ...
    useEffect(() => {
        if (!isLoading && isAuthenticated && isAuthorized && hasOrganization) {
            fetchUsers()
        }
    }, [isLoading, isAuthenticated, isAuthorized, hasOrganization])
                ...
                <div className="mb-4 flex justify-between items-center">
                    <div className="relative w-64">
                        ...
                        <input
                            ...
                            className="pl-10 p-2 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm"
                        />
                    </div>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {filteredUsers.map((orgUser) => (
                                    <tr key={orgUser.id}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span
                                                className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${orgUser.isActive ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"
                                                    }`}
                                            >
